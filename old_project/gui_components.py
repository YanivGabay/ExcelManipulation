...
def create_top_frame(parent, load_file_func, load_excel_orgin_file_func,save_excel_func):
    ...
    load_button = ttk.Button(top_frame, text="Load Txt File Manually", command=load_file_func)
    load_button.pack(side='left', padx=10, pady=10)

    excel_load_file = ttk.Button(top_frame, text="Load Excel File", command=load_excel_orgin_file_func)
    excel_load_file.pack(side='left', padx=10)

    save_excel_button = ttk.Button(top_frame, text="Save to Excel", command=save_excel_func)
